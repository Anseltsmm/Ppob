...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
    <h5 class="fw-bold mb-0"><i class="bi bi-receipt text-primary me-1"></i> Transaksi</h5>
</div>
...
<form method="GET" action="{{ route('admin.orders.index') }}" class="mb-3">
    ...
    <div class="search-bar">
        ...
        <input type="text" name="q" value="{{ $search ?? '' }}" placeholder="Cari kode order, produk, customer, atau nomor..." autocomplete="off">
    </div>
</form>

{{-- Date Filter --}}
<form method="GET" action="{{ route('admin.orders.index') }}" class="mb-3" id="dateFilterForm">
    @if($activeStatus !== 'all')
        <input type="hidden" name="status" value="{{ $activeStatus }}">
    @endif
    @if($search)
    ...
    <div class="d-flex align-items-center gap-2 flex-wrap">
        ...
        @if($dateFrom || $dateTo)
            <a href="{{ route('admin.orders.index', array_filter(['status' => $activeStatus !== 'all' ? $activeStatus : null, 'q' => $search])) }}"
...
<div class="filter-tabs mb-3">
    <a href="{{ route('admin.orders.index', array_filter(['q' => $search, 'date_from' => $dateFrom, 'date_to' => $dateTo])) }}"
       class="filter-tab {{ $activeStatus === 'all' ? 'active' : '' }}">
        Semua <span class="count">{{ $counts['all'] }}</span>
    </a>
    <a href="{{ route('admin.orders.index', array_filter(['status' => 'success', 'q' => $search, 'date_from' => $dateFrom, 'date_to' => $dateTo])) }}"
    ...
    <a href="{{ route('admin.orders.index', array_filter(['status' => 'pending', 'q' => $search, 'date_from' => $dateFrom, 'date_to' => $dateTo])) }}"
    ...
    <a href="{{ route('admin.orders.index', array_filter(['status' => 'failed', 'q' => $search, 'date_from' => $dateFrom, 'date_to' => $dateTo])) }}"
    ...
    @foreach($grouped as $label => $items)
        ...
        @foreach($items as $order)
            @php
                ...
                $profit = (float) $order->sell_price - (float) $order->buy_price;
            @endphp
            <a href="{{ route('admin.orders.show', $order) }}" class="order-card">
                ...
                <div class="order-body">
                    <div class="order-top">
                        <div>
                            ...
                            <div class="order-dest">
                                ...
                                &middot; <i class="bi bi-hash"></i>{{ $order->destination }}
                            </div>
                        </div>
                        <div class="text-end">
                    ...
                    <div class="order-bottom">
                        <div>
                            ...
                            @if($profit > 0)
                                <span style="font-size:.7rem; color:#16a34a; margin-left:4px;">+Rp {{ number_format($profit, 0, ',', '.') }}</span>
                            @endif
                        </div>
...
@else
    <div class="text-center py-5 text-muted">
        ...
        @if($search || $activeStatus !== 'all' || $dateFrom || $dateTo)
            <p class="mb-2">Tidak ada transaksi yang cocok.</p>
            <a href="{{ route('admin.orders.index') }}" class="btn btn-outline-primary btn-sm rounded-pill">
